...
export default class Timer extends Component {
  ...
  totalEarned () {
    return this.state.hourlyRate * this.totalTime().as('hours')
  }
    ...
    return (
      <div className='flex flex-col bg-white shadow-lg relative rounded'>
        ...
        <div className={'absolute pin bg-white ' + (this.state.showConfiguration ? '' : 'hidden')}>
          <div className='flex flex-col justify-between h-full'>
            <div className='p-4' onClick={this.configure}>
              <i className='float-right text-red-lighter cursor-pointer fas fa-times-circle' />
            </div>
            <div className=''>
              <div className='bg-grey-lighter mx-6 p-4 rounded'>
              ...
              <div className='bg-grey-lighter mx-6 p-4 rounded'>
                <label className='block text-grey-darker text-sm font-bold pb-2' htmlFor='break-unit-time'>
                  Hourly Rate <span className='text-sm font-normal'>(in dollars)</span>
                </label>
                <div className='flex flex-row justify-between items-center'>
                  <input className='appearance-none border rounded w-3/5 py-2 px-3 text-grey-darker' id='break-unit-time' type='text' placeholder='3600'
                    onChange={this.changeHourlyRate} value={this.state.hourlyRate} />
                  <span className='text-grey-darkest'>{this.hourlyRate}</span>
                </div>
              </div>
            </div>
            <div className='p-8'>
        ...
        <div className='p-4'>
          ...
          <span className='block py-2 text-grey-dark'>Total Earned : ${this.totalEarned().toFixed(2)}</span>
        </div>
      </div>
    )
  }
